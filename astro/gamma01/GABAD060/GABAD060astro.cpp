...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss>=1000000 && ss<=1100000 && ss%100 == 0){
            char s3[255] = "v_Isyn_GABAD060_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f %4f\n",v0[N_middle][N_middle],Isy[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_GABAD060_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][50],Isy[44][50]);
        ...
        if(ss%(10000*100)==0){
			...
			char s5[255];
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvGABAD060.txt", ss/100);
            sprintf(s1, "%dnGABAD060.txt", ss/100);
            sprintf(s2, "%dsGABAD060.txt", ss/100);
            sprintf(s5, "%dIsyGABAD060.txt", ss/100);
