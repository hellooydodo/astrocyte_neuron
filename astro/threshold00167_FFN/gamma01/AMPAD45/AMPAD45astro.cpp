...
double FFN=0;
double Isy1=9.897e-2,Isy2=4.147e-1;
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_AMPAD45_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_AMPAD45_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][55],Isy[44][55]);
            fclose(fp8);
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvAMPAD45.txt", ss/100);
            sprintf(s1, "%dnAMPAD45.txt", ss/100);
            sprintf(s2, "%dsAMPAD45.txt", ss/100);
			...
			FFN /=(N*N);
			char s5[255];
			sprintf(s5, "%dFFN_AMPAD45.txt", ss/100);
			fp9 = fopen(s5,"a+");
            fprintf(fp9,"%.4f",FFN);
            fclose(fp9);
            FFN = 0; 
