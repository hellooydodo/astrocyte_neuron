...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss>=190000 && ss<=200000 && ss%100 == 0){
            char s3[255] = "v_GABAD15_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_GABAD15_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][55],Isy[44][55]);
            fclose(fp8);
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvGABAD15.txt", ss/100);
            sprintf(s1, "%dnGABAD15.txt", ss/100);
            sprintf(s2, "%dsGABAD15.txt", ss/100);
