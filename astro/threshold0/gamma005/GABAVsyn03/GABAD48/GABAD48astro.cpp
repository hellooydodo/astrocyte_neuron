...
///////initial conditions//////////////////////
   ...
   for(int i = 1; i <= N;i++){ //（4）
    ...
    double const g_s = 1.2; // Isy,相当于D/4 
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_GABAD48_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_GABAD48_middle2.txt";
            fp8 = fopen(s4,"a+");
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvGABAD48.txt", ss/100);
            sprintf(s1, "%dnGABAD48.txt", ss/100);
            sprintf(s2, "%dsGABAD48.txt", ss/100);
