...
///////initial conditions//////////////////////
   ...
   for(int i = 0; i <= N+1;i++){ //（4）
    ...
    double const g_s = 1.1/4; // Isy,相当于D
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss>=1000000 && ss<=1100000 && ss%100 == 0){
            char s3[255] = "v_Isyn_AMPAD110_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f %4f\n",v0[N_middle][N_middle],Isy[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_AMPAD110_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][50],Isy[44][50]);
            fclose(fp8);
        ...
        if(((ss < (10000*100))&& (ss%(2000*100)==0)) || ss%(10000*100)==0){
			...
			char s5[255];
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvAMPAD110.txt", ss/100);
            sprintf(s1, "%dnAMPAD110.txt", ss/100);
            sprintf(s2, "%dsAMPAD110.txt", ss/100);
            sprintf(s5, "%dIsyAMPAD110.txt", ss/100);
            ...
            fclose(fp10);
        	char s7[255];
			sprintf(s7,"%dCaAMPAD110.txt",ss/100);
			fp11 = fopen(s7,"w");
        	for(int i = 1;i <= N;i++){
                for(int j = 1;j < N;j++){
            ...
            fclose(fp11);
        	char s8[255];
			sprintf(s8,"%dIPAMPAD110.txt",ss/100);
			fp12 = fopen(s8,"w");
        	for(int i = 1;i <= N;i++){
                for(int j = 1;j < N;j++){
