...
///////initial conditions//////////////////////
   ...
   for(int i = 0; i <= N+1;i++){ //（4）
    ...
    double const g_s = 0.35/4; // Isy,相当于D
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss>=1000000 && ss<=1100000 && ss%100 == 0){
            char s3[255] = "v_Isyn_GABAD035_middle.txt";
            fp7 = fopen(s3,"a+");
        ...
        if(((ss < (10000*100))&& (ss%(2000*100)==0)) || ss%(10000*100)==0){
			...
			char s5[255];
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvGABAD035.txt", ss/100);
            sprintf(s1, "%dnGABAD035.txt", ss/100);
            sprintf(s2, "%dsGABAD035.txt", ss/100);
            sprintf(s5, "%dIsyGABAD035.txt", ss/100);
