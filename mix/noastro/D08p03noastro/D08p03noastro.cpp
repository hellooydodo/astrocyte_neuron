...
///////initial conditions//////////////////////
   ...
   for(int i = 1; i <= N;i++){ //（4）
    ...
    double const g_s = 0.2; // Isy,相当于D/4 
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_p03D08_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_p03D08_middle2.txt";
            fp8 = fopen(s4,"a+");
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvp03D08.txt", ss/100);
            sprintf(s1, "%dnp03D08.txt", ss/100);
            sprintf(s2, "%dsp03D08.txt", ss/100);
