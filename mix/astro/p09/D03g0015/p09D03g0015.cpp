...
///////initial conditions//////////////////////
   ...
   for(int i = 1; i <= N;i++){ //（4）
    ...
    double const g_s = 0.075; // Isy,相当于D/4 
    double const tau_ca = 6, kappa = 0.5, Ca_th = 0.2; //f
    double const gamma = 0.15, P = 0.8; //Iast
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_p09D03g015_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_p0903g015_sample.txt";
            fp8 = fopen(s4,"a+");
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvp09D03g015.txt", ss/100);
            sprintf(s1, "%dnp09D03g015.txt", ss/100);
            sprintf(s2, "%dsp0903g015.txt", ss/100);
