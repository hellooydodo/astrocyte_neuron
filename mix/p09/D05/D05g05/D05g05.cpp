...
///////initial conditions//////////////////////
   ...
   for(int i = 0; i <= N+1;i++){ //（4）
    ...
    double const g_s = 0.5/4; // Isy,相当于D
    double const tau_ca = 6, kappa = 0.5, Ca_th = 0.2; //f
    double const gamma = 0.5, P = 0.8; //Iast
        ...
        for(int i = 1; i <= N; i++){
            for (int j = 1; j <= N; j++){
                ...
                if (astro && i<N && j<N){ // 胶质细胞相关
                	...
                	I_ast[i][j] = gamma*P*(ampa[i-1][j-1]*f0[i-1][j-1] + ampa[i][j-1]*f0[i][j-1] + ampa[i-1][j]*f0[i-1][j] + ampa[i][j]*f0[i][j]);
                ...
                Isy[i][j] = -g_s*(v0[i][j] - Vsyn[i][j])*(g0[i-1][j] + g0[i][j-1] + g0[i+1][j] + g0[i][j+1]); //统一了羊师兄和王荣师兄的形式
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss>=1000000 && ss<=1100000 && ss%100 == 0){
            char s3[255] = "v_Isyn__middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f %4f\n",v0[N_middle][N_middle],Isy[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn__sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][50],Isy[44][50]);
            fclose(fp8);
        ...
        if(((ss < (10000*100))&& (ss%(2000*100)==0)) || ss%(10000*100)==0){
			...
			char s5[255];
                 //scanf_s("%d",&ss);
            sprintf(s, "%dv.txt", ss/100);
            sprintf(s1, "%dn.txt", ss/100);
            sprintf(s2, "%ds.txt", ss/100);
            sprintf(s5, "%dIsy.txt", ss/100);
        	...
        	for(int i = 1;i <= N;i++){
                ...
                fprintf(fp10,"%.4f",Isy[i][N]);
                fprintf(fp10,"\n");
            }
            fclose(fp10);
        	char s7[255];
			sprintf(s7,"%dCa.txt",ss/100);
			fp11 = fopen(s7,"w");
        	for(int i = 1;i <= N;i++){
                for(int j = 1;j < N;j++){
