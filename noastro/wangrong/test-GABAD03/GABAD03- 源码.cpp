...
double f(double v,double n,double Iapp,double Isy)
{
	...
	Ica=1.1*0.5*(1+tanh((v+0.01)/0.15))*(v-1);//na电导
	Ik=2*n*(v+0.7);//k电导项
	Il=0.5*(v+0.5);//漏电导项
...
{
	double result,tn,nn;
	tn=1/cosh((v-0.0167)/0.5);
	nn=0.5*(1+tanh((v-0.0167)/0.25));
	result=0.04*(nn-n)/tn;
	return result;
}
...
//    fp8=fopen("synaptic.dat","wb");
   ...
   /* for(i=1;i<=N;i++) //错误，v0和v用混，这里直接把初值覆盖了
	...
	while (delta_sumV>-1)
    {
	  times+=step;
	  ss=ss+1;
	  double Iex= 0.22916;//the external current
