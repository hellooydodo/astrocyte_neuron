...
///////initial conditions//////////////////////
   ...
   for(int i = 0; i <= N+1;i++){ //（4）
    ...
    double const g_s = 0.025; // Isy,相当于D
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_AMPAD01_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_AMPAD01_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][55],Isy[44][55]);
            fclose(fp8);
        ...
        if(ss%(10000*100)==1){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvAMPAD01.txt", ss/100);
            sprintf(s1, "%dnAMPAD01.txt", ss/100);
            sprintf(s2, "%dsAMPAD01.txt", ss/100);
