...
///////initial conditions//////////////////////
   ...
   for(int i = 1; i <= N;i++){ //（4）
    ...
    double const g_s = 0.0375; // Isy,相当于D
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_AMPAD015_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_Isyn_AMPAD015_sample.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f %4f\n",v0[44][55],Isy[44][55]);
            fclose(fp8);
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvAMPAD015.txt", ss/100);
            sprintf(s1, "%dnAMPAD015.txt", ss/100);
            sprintf(s2, "%dsAMPAD015.txt", ss/100);
