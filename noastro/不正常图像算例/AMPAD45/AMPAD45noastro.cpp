...
///////initial conditions//////////////////////
   ...
   for(int i = 1; i <= N;i++){ //（4）
    ...
    double const Iex = 0.1375;//the external current
    double const cm = 1, phi = 0.04; //v,n
    double const c1 = 0.185, v1 = 6, v2 = 0.11, v3 = 0.9e-6, k3 = 0.1e-6; // Jchannel,Jpump,Jleak;
	double const d1 = 0.13e-6, d2 = 1.049e-6, d3 = 0.9434e-6, d5 = 0.08234e-6, a2 = 0.2e-6, c0 = 2e-6; //m_inf,n_inf,alpha_q,beta_q,Ca_er ...
	//数据来自 Spontaneous Oscillations of Dressed neurons，羊师兄
    double const Ip3_st = 0.16e-6, tau_Ip3 = 7,r_ip3 = 7.2e-3;
    ...
    double const g_s = 4.5; // Isy,相当于D
    double const tau_ca = 6, kappa = 0.5, Ca_th = 0.2e-6; //f
        ...
        for(int i = 1; i <= N; i++){
            for (int j = 1; j <= N; j++){
                ...
                if (astro && i<N && j<N){ // 胶质细胞相关
                	...
                	I_ast[i][j] = gamma*P*(f0[i][j] + f0[i][j+1] + f0[i+1][j] + f0[i+1][j+1]);
				}			
...
//         if(ss>=800000&&ss%10==0)
	//		{
        ...
        if (ss%100 == 0){
            char s3[255] = "v_AMPAD25_middle.txt";
            fp7 = fopen(s3,"a+");
            fprintf(fp7,"%.4f\n",v0[N_middle][N_middle]);
            fclose(fp7);
            char s4[255] = "v_AMPAD25_middle2.txt";
            fp8 = fopen(s4,"a+");
            fprintf(fp8,"%.4f\n",v0[N_middle/2][N_middle/2]);
            fclose(fp8);
        ...
        if(ss%(10000*100)==0){
            ...
            char s2[255];
                 ...
                 //scanf_s("%d",&ss);
            sprintf(s, "%dvAMPAD25.txt", ss/100);
            sprintf(s1, "%dnAMPAD25.txt", ss/100);
            sprintf(s2, "%dsAMPAD25.txt", ss/100);
