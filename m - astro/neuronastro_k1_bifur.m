...
fid=fopen('Astrocyte_k1_bifur.dat','w');
...
theta_s=0.2;sigma_s=0.02;phi=1.15;r_IP3=7.2;
...
time = 0:dt:tEnd;
  ...
  I_slow = zeros(length(time),1);           I_slow(1) = 0.02;
      for i=2:length(time)
        ...
        f(i) = f(i-1) + (-f(i-1)./tau_Ca+(1-f(i-1))*kappa*(Ca(i-1)-Ca_th))*dt;
      end
      [pks1,locs1]=findpeaks(V(tEnd*90:end),'minpeakdistance',14000);
      [pks2,locs2]=findpeaks(-V(tEnd*90:end),'minpeakdistance',14000);
