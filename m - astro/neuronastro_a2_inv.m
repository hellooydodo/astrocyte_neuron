...
fid=fopen('Astrocyte_k1_inv.dat','w');
...
theta_s=0.2;sigma_s=0.02;phi=1.15;r_IP3=100;
g_amma=0.5;tau_IP3=7; tau_Ca=6;kappa=0.5;Ca_th=0.2;
% 	***************************************************************************/
...
time = 0:dt:tEnd;
  ...
  for k1 = 1 : -0.001 :0
      ...
      [pks,locs]=findpeaks(V(500000:end));
      
       for i = 3 : length(locs)-1
           fprintf(fid,'%12.8f %12.8f\n',k1, (locs(i)-locs(i-1))./100000);
       end
       
        V(1) = V(length(V));w(1) =w(length(V)); Fai(1) = Fai(length(V));IP3(1) = IP3(length(V));  Ca(1) = Ca(length(V));  q(1) = q(length(V)); f(1) = f(length(V));I_slow(1) = I_slow(length(V));
