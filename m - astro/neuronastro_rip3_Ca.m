...
fid1=fopen('Astrocyte_rip3_Camax.dat','w');
fid2=fopen('Astrocyte_rip3_Camin.dat','w');
...
theta_s=0.2;sigma_s=0.02;phi=1.15;
% r_IP3=100;
...
time = 0:dt:tEnd;
  ...
  I_slow = zeros(length(time),1);           I_slow(1) = 0.02;
      ...
      [pks1,locs1]=findpeaks(Ca(tEnd*80:end));
      [pks2,locs2]=findpeaks(-Ca(tEnd*80:end));
      pks3=-pks2;
       for i = 1 : length(pks1)
           fprintf(fid1,'%12.8f %12.8f\n',r_IP3, pks1(i)*1000);
       end
       for i = 1 : length(pks3)
