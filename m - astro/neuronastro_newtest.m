...
for i=2:length(time)
        ...
        V(i) = V(i-1) + (1./Cm*(-g_Ca*m_inf*(V(i-1)-v_Ca)-g_K*w(i-1)*(V(i-1)-v_K)-g_L*(V(i-1)-v_L)+I_slow(i-1)+I_ext+-k1*(0.1+0.06*Fai(i-1).^2).*V(i-1)))*dt;%

          w_inf=0.5*(1+tanh((V(i-1)-v3)./v4));
          tau_w=1./(cosh((V(i-1)-v3)/(2*v4)));
        w(i) = w(i-1) + (phi*(w_inf-w(i-1))./tau_w)*dt;
        I_slow(i) = I_slow(i-1) + (0.00*(-0.22-V(i-1)-alpha*I_slow(i-1)))*dt;

        Fai(i) = Fai(i-1) + (k3*V(i-1)-k2*Fai(i-1)+A*cos(omiga*i))*dt;%

end

neuronastro_k1_plot_noastro();
